                                 ...
                                 set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64& nValueRet) const
{
    ...
    BOOST_FOREACH(COutput output, vCoins)
    {
        ...
        else if (n < nTargetValue + SMALL_TRANSACTION_TRESHOLD)
        {
    ...
    if (nBest != nTargetValue && nTotalLower >= nTargetValue + SMALL_TRANSACTION_TRESHOLD)
        ApproximateBestSubset(vValue, nTotalLower, nTargetValue + SMALL_TRANSACTION_TRESHOLD, vfBest, nBest, 1000);

    // If we have a bigger coin and (either the stochastic approximation didn't find a good solution,
    //                                   or the next bigger coin is closer), return the bigger coin
    if (coinLowestLarger.second.first &&
        ((nBest != nTargetValue && nBest < nTargetValue + SMALL_TRANSACTION_TRESHOLD) || coinLowestLarger.first <= nBest))
    {
        setCoinsRet.insert(coinLowestLarger.second);
        nValueRet += coinLowestLarger.first;
    }
...

    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            loop
            {
                ...
                if (nFeeRet < CTransaction::nMinTxFee && nChange > 0 && nChange < SMALL_TRANSACTION_TRESHOLD)
                {
